...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const tribe = new Tribe({ name, description, category, createdBy: req.userId, members: [req.userId] });
    await tribe.save();
...
router.post('/:id/join', authMiddleware, async (req, res) => {
  try {
    const tribe = await Tribe.findByIdAndUpdate(req.params.id, { $addToSet: { members: req.userId } }, { new: true });
    res.json(tribe);
  } catch (error) {
...
router.post('/:id/leave', authMiddleware, async (req, res) => {
  try {
    const tribe = await Tribe.findByIdAndUpdate(req.params.id, { $pull: { members: req.userId } }, { new: true });
    res.json(tribe);
  } catch (error) {
...
router.get('/search/:query', async (req, res) => {
  try {
    const tribes = await Tribe.find({ isPublic: true, $or: [{ name: { $regex: req.params.query, $options: 'i' } }, { description: { $regex: req.params.query, $options: 'i' } }] });
    res.json(tribes);
  } catch (error) {
